...
<div class="col-md-12">
<?php
$query ='SELECT DISTINCT leadName, leadDescription FROM businesslead where businessleadid = '.$_POST['id'].'';
          $stmtfetch = $db->prepare($query);
          $stmtfetch->execute();
          $row = $stmtfetch->fetch(PDO::FETCH_GROUP);
?>

<label>Lead Name: </label><input type="text" class="form-control " value="<?php echo $row[0];?>" id="leadname" disabled/><br>


<label>Lead Description:</label> <textarea rows="4" class="form-control " cols="50" id="leaddesc" disabled><?php echo $row[1]?></textarea><br>
<?php
$query ='SELECT imagePath, businessLeadImageID from businessleadimage  where businessleadid = '.$_POST['id'].'';
          $stmtfetch = $db->prepare($query);
          $stmtfetch->execute();
          $i=0;
