...
<?php
    ...
    if(isset($_POST['submit'])){
      ...
      $stmt = $db->prepare('SELECT password FROM manager WHERE username = :username');
      $stmt->execute(array(':username' => $_SESSION['manager']));
      ...
      if(!isset($error)){
    		...
    		if($user->password_verify($_POST['curpass'],$row['password']) == 1)
        {
          ...
          $stmt = $db->prepare('update manager set password = :password  WHERE username = :username');
      		$stmt->execute(array(':username' => $_SESSION['manager'],
